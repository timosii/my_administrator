...
async def choose_fil_handler(message: Message,
    ...
    for check in checks:
        text_mes = MoPerformerMessages.view_check_info(
            check_fil_=check.fil_,
            check_date=check.mfc_finish,
            violations_count=len(await violation_obj.get_violations_found_by_check(check_id=check.id))
        )
...
@router.callback_query(F.data.startswith('violations_'),
                       StateFilter(MoPerformerStates.mo_performer))
async def get_violations(callback: CallbackQuery,
                         violation_obj: ViolationFoundService = ViolationFoundService()):
    ...
    for violation in violations:
        ...
        keyboard = MoPerformerKeyboards().get_under_violation_photo(violation_id=violation.id)
        text_mes = MoPerformerMessages.view_violation_info(
            violation_zone=zone,
            violation_name=violation_name,
            violation_detected=violation.violation_detected,
            violation_comm=violation.comm if violation.comm else 'Комментарий отсутствует'
        )
...
async def process_comment_callback(
    ...
    await callback.message.edit_text(
        ...
        reply_markup=MoPerformerKeyboards().get_under_violation_photo(violation_id=violation.id)
        )
    await callback.answer()
...
async def process_photo_callback(
    ...
    violation_found = await violation_obj.get_violation_found_by_id(violation_id=violation_id)
